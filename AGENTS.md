...
- **会话复用**：挑战通过后默认 sticky 留在 L3 或 L4。CDP：过验证后 Cookie 同步到 L1/L2，内容优先走 L2（避免反复 `goto` 抢焦点）；若 L2 仍挑战则回退并 sticky L3（不切 headless）。配置了 CDP 时，L1 `httpx.ConnectError`（直连 RST 等）也会升 L3；其后若 L2 仍失败则 sticky L3。
